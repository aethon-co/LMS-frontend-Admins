...
export const DashboardLayout: React.FC = () => {
  ...
  const navItems = role === 'admin' 
    ...
    : [
        { label: 'Overview', icon: LayoutDashboard, path: '/tutor/dashboard' },
      ];
    ...
    <div className="flex bg-slate-50 dark:bg-[#0c0e12] min-h-screen font-sans text-left text-slate-900 dark:text-[#f0f2f5] w-full transition-colors duration-200">
      ...
      <aside className="w-64 bg-white dark:bg-[#13151a] border-r border-slate-200 dark:border-[#242830] hidden md:flex flex-col">
        ...
        <nav className="flex-1 p-4 space-y-1">
          {navItems.map((item) => {
            const active = location.pathname === item.path || (item.path !== `/admin/dashboard` && location.pathname.startsWith(item.path) && item.path !== '/tutor/dashboard' && item.path !== '/admin/dashboard');
            return (
              <Link 
                key={item.path} 
                to={item.path}
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden">
        <header className="h-16 bg-white dark:bg-[#13151a] border-b border-slate-200 dark:border-[#242830] flex items-center px-8 sticky top-0 z-10 w-full shrink-0">
          <h1 className="text-xl font-semibold m-0 p-0 tracking-tight text-slate-900 dark:text-[#f0f2f5] capitalize">
            {location.pathname.includes('course') ? 'Course Details' : location.pathname.includes('batch') ? 'Batch Details' : 'Overview'}
          </h1>
        </header>
