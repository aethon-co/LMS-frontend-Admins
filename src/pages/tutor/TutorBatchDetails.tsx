...
import { 
  ...
  getBatchStudentStats
} from '../../api';
import { ArrowLeft, Users, Calendar, Plus, FileText, CheckCircle2, ClipboardList, UserCheck, UserX, Loader2, Search, X } from 'lucide-react';
...
export const TutorBatchDetails: React.FC = () => {
  ...
  // Build a map from studentId -> stats for the Students tab
  ...
  return (
    <div className="min-h-screen bg-slate-950 font-sans text-slate-100">
      ...
      <div className="max-w-7xl mx-auto p-8">
            ...
            {isLoadingAssignments ? (
              ...
              <div className="space-y-4">
                {assignments.map((assignment: any) => (
                  <div key={assignment._id} className="group rounded-2xl bg-slate-800/50 p-6 border border-slate-700/50 hover:border-slate-600 transition-all">
                  ...
                  return (
                    <div key={student.studentId} className="rounded-2xl bg-slate-800/50 border border-slate-700/50 p-5 hover:border-slate-600 transition-all">
                      ...
                      <div className="w-full bg-slate-700 rounded-full h-1.5 overflow-hidden">
                        <div className={`${barColor} h-1.5 rounded-full transition-all duration-500`} style={{ width: `${student.percentage}%` }}></div>
                      </div>
                    </div>
                  );
                })}
        ...
        {/* ─── Attendance Tab (with date filter) ─── */}
        {activeTab === 'attendance' && (
          <div>
            {/* Date Filter */}
